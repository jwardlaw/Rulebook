...
public partial class APCharacterController : MonoBehaviour
{
	...
	void StateMeleeAttack(APFsmStateEvent eEvent)
	{
		...
		case APFsmStateEvent.eEnter:
		{
			...
			m_eventListeners.ForEach(e => e.OnMeleeAttackStart(curAttack));
		}
		break;
		...
		case APFsmStateEvent.eUpdate:
		{
				...
				if (curHitZone.m_active && curHitZone.gameObject.activeInHierarchy)
				{
					...
					for (int i = 0; i < hitCount; i++)
					{
						...
						if (hitable != null && !curHitZone.attackHits.Contains(hitable))
						{
							...
							{
								m_eventListeners.ForEach(e => e.OnMeleeAttackHit(curAttack, hitable));
			...
			if ((fAnimTime >= 1f) && !IsNewStateRequested())
			{
		...
		{
			APMeleeAttack curAttack = m_meleeAttacks.m_attacks [m_curAttackId];
			m_eventListeners.ForEach(e => e.OnMeleeAttackEnd(curAttack));
			m_curAttackId = -1;
		}
		break;
		}
	...
	void HandleMeleeAttack()
	{
		...
		if (m_meleeAttacks.m_enabled && (bCrouched || (GetState() == State.Standard)) && !IsNewStateRequested())
		{
			...
			for (int i = 0; i < m_meleeAttacks.m_attacks.Length; i++)
			{
				...
				if (curAttack.m_button.GetButtonDown())
				{
	...
	void StateRangedAttack(APFsmStateEvent eEvent)
	{
		...
		case APFsmStateEvent.eEnter:
		{
			...
			m_eventListeners.ForEach(e => e.OnRangedAttackStart(curAttack));
		}
		break;
		...
		case APFsmStateEvent.eUpdate:
		{
			...
			if ((fAnimTime >= 1f) && !IsNewStateRequested())
			{
				// stay in same state if player is still firing, just reset context and animation
				APRangedAttack curAttack = m_rangedAttacks.m_attacks[m_curAttackId];
				if (curAttack.m_button.GetButtonDown())
				{
		...
		{
			APRangedAttack curAttack = m_rangedAttacks.m_attacks [m_curAttackId];
			m_eventListeners.ForEach(e => e.OnRangedAttackEnd(curAttack));
			m_curAttackId = -1;
		}
		break;
		}
	...
	void AlignGroundAxis()
	{
        ...
        GetComponent<Rigidbody2D>().angularVelocity = angleSign * Mathf.Min(fAngle / Time.deltaTime, m_groundAlign.m_rotateSpeed);
	}
	...
	void HandleJump()
	{
		...
		{ 
			if(m_onGround || (m_jump.m_airJumpCount > m_airJumpCount))
			{
				...
				{ 
					...
					{
						// check if down jumping on oneway ground
						if((m_inputs.m_axisY.GetValue() < 0f) && m_groundAlign.m_oneWayDownJump && GetMotor().IgnoreOnOneWayGround())
						{
							Uncrouch();
							return;
						}
		...
		if(m_lastJumpTime > 0f && !m_hasReleasedJump)
		{
			m_hasReleasedJump = !m_jump.m_button.GetButton() || m_motor.TouchHead || m_onGround;
	...
	public void Jump(float fMinHeight, float fMaxHeight)
	{
		...
		else
		{
			...
			m_hasReleasedJump = false; // to stop high jump as soon as button is released
	...
	{
		// early exit
		if (!m_jump.m_button.IsSpecified() || !m_jump.m_button.GetButtonDown() || IsCrouched() || m_onGround || IsNewStateRequested())
			return;
	...
	bool CanUncrouch()
	{
		...
		for (int i = 0; i < hitCount; i++)
		{
			if(!m_overlapResult[i].isTrigger && m_overlapResult[i] != GetComponent<Collider2D>())
				return false;
		}
		return true;
	}
	...
	void StateShift(APFsmStateEvent eEvent)
	{
		...
		case APFsmStateEvent.eEnter:
		{
			...
			m_shiftDuration = m_curShift.m_pauseTime + m_curShift.m_moveLength / m_curShift.m_moveSpeed;
			PlayAnim(m_curShift.m_anim, 0f);

...
public class APInputButton
{
	...
	bool m_bUp = false;
	float m_timeDown = 0f;
	static float m_downPushDuration = 0.1f;		// time during which down button status can lasts even if player has released button
	...
	{
		bool bRet = string.IsNullOrEmpty(m_name) ? false : Input.GetButton(m_name) && Holders() && Releasers();
		if(m_plugin != null)
		{
	...
	public void Refresh(bool bSet, bool bAutoFire) //= false)
	{
		...
		{
			if(m_bDown && (Time.time >= m_timeDown + m_downPushDuration) && (!bAutoFire || !GetButton()))
			{
				m_bDown = false;
			}

...
public class APEdgeGrab : MonoBehaviour 
{
    ...
    public class GrabInAirDesc
    {
        ...
        public string m_anim;                       // player's animation state to launch when grabbing directly
    };
    ...
    public class ExitClimbUpDesc
    {
        ...
        public string m_anim;                        // player's animation state to launch when exiting by climbing up
    ...
    public class GrabFromTopDesc
    {
        ...
        public string m_anim;                       // player's animation state to launch when grabbing from top of edge
    };
		...
		if(m_controlled)
		{
			...
			else
			{
                ...
                if (m_exitClimbUp.m_enabled && m_exitClimbUp.m_button.IsSpecified() && m_exitClimbUp.m_button.GetButton())
				{
                    ...
                    StartAnimation(m_exitClimbUp.m_anim);
				}
			}
		}
	}
	...
	void MatchGrabAnchor()
	{
		...
		{
			m_controlled.GetComponent<Rigidbody2D>().velocity = v2Dist / Time.fixedDeltaTime;
		}
		else
		{
			m_controlled.GetComponent<Rigidbody2D>().velocity = Vector2.zero;
		}
	}
    ...
    void StartGrab(APCharacterController controlled, string sAnim, Vector2 grabHandle)
    {
        ClearRuntimeValues();

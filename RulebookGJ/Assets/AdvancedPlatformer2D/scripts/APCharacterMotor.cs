...
public class APCharacterMotor : MonoBehaviour 
{
	...
	{
		Vector2 rayOrigin = transform.TransformPoint(Vector2.Scale(ray.m_position , m_scale) + m_offset);
		return rayOrigin;
	}
	...
	public void Move()
	{
		...
		GetComponent<Rigidbody2D>().velocity = (m_curPos - v2InitialPos) / Time.deltaTime; 
	...
	bool IsOneWayGround(Collider2D collider)
	{
	...
	public bool IgnoreOnOneWayGround()
	{
		...
		foreach(CharacterRay2D curRay in m_RaysGround)
		{
			...
			{
				if(curRay.m_hitInfo.m_penetration < 0.1 && IsOneWayGround(curRay.m_hitInfo.m_collider))
				{
					bIgnored = true;
		...
		for(int i = 0; i < rays.Length; i++)
		{
			...
			if(hitCount > 0)
			{
				...
				for(int hitId = 0; hitId < hitCount; hitId++)
				{
					...
					if(hit.rigidbody == GetComponent<Rigidbody2D>())
						continue;
					...
					if(hit.collider != null)
					{
						...
						if((!bDown || (bDown && hit.normal.y < 0f)) && IsOneWayGround(hit.collider))
							continue;
					}
